...
const Cell = ({ className, children }) => <td className={className}>{children}</td>;
...
function Row({ label, id, selected, deleteFunc, selectFunc }) {
  return (
    <tr className={selected ? 'danger' : null}>
      <Cell className='col-md-1' $HasTextChildren>{id}</Cell>
      <Cell className='col-md-4'><a onClick={linkEvent(id, selectFunc)} $HasTextChildren>{label}</a></Cell>
      <Cell className='col-md-1'><a onClick={linkEvent(id, deleteFunc)}><RemoveIcon preload={false} /></a></Cell>
