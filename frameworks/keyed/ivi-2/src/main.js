...
const Cell = component(() => {
  return ({ className, children }) => td(className, _, children);
});
const Row = component((c) => {
  ...
  return (item) => (_item = item,
    tr(isSelected(item) ? "danger" : "", _, [
      ...
      Cell({ className: "col-md-4", children: Events(s, a(_, _, item.label)) }),
      Cell({ className: "col-md-1", children: Events(r, RemoveButton) }),
